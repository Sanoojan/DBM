...
SAVE_ROOT = Path("./flow_only_models")
SAVE_ROOT.mkdir(exist_ok=True)

T          = 200  # training clip length
INF_T      = 1000 # inference clip length (middle of video)
BATCH_SIZE = 4
EPOCHS     = 20
LR         = 1e-4
...
class FlowOnlyDataset(Dataset):
    ...
    def __init__(self, samples, t=T, train=True, inference=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.inference:
            start = max(0, (len(flow_files) - INF_T)//2)
            t = INF_T
        else:
            t = self.t
            start = random.randint(0, len(flow_files) - t) if self.train else (len(flow_files) - t)//2

        # Flow clip: (T,H,W,2) → (2,T,H,W)
        flows = np.stack([np.load(str(f)) for f in flow_files[start:start+t]])
...
class FlowOnlyModel(nn.Module):
    ...
    def forward(self, flow):
        ...
        return self.classifier(f_feat).view(-1)

# =========================================================
# TRAIN / VAL
# =========================================================
def make_loader(samples, shuffle=True, train=True, inference=False):
    dataset = FlowOnlyDataset(samples, train=train, inference=inference)
    return DataLoader(
        dataset,
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(participants, labels)):
    ...
    for epoch in range(EPOCHS):
        ...
        tr_loss,tr_f1,tr_auc,tr_uar,_ = run_epoch(model, train_loader, criterion, optimizer, True)
        print(f"[Train] loss={tr_loss:.4f} f1={tr_f1:.4f} auc={tr_auc:.4f} uar={tr_uar:.4f}")

        vl_loss,vl_f1,vl_auc,vl_uar,th = run_epoch(model, val_loader, criterion, None, False)
