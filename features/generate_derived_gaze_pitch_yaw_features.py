...
def process_sample(dataset, sample_number, base_out_dir):
    ...
    for side in ["left", "right"]:
        gaze_x_origin = tobii_df[f"tobii_{side}_eye_gaze_origin_in_user_x"].to_numpy()
        gaze_x_pt = tobii_df[f"tobii_{side}_eye_gaze_pt_in_user_x"].to_numpy()
        gaze_y_origin = tobii_df[f"tobii_{side}_eye_gaze_origin_in_user_y"].to_numpy()
        gaze_y_pt = tobii_df[f"tobii_{side}_eye_gaze_pt_in_user_y"].to_numpy()
        gaze_z_origin = tobii_df[f"tobii_{side}_eye_gaze_origin_in_user_z"].to_numpy()
        gaze_z_pt = tobii_df[f"tobii_{side}_eye_gaze_pt_in_user_z"].to_numpy()

        # Calculate pitch and yaw
        output_df[f"gaze_{side}_pitch"] = np.rad2deg(np.atan2(gaze_y_pt - gaze_y_origin, gaze_z_origin - gaze_z_pt))
        output_df[f"gaze_{side}_yaw"] = np.rad2deg(np.atan2(gaze_x_pt - gaze_x_origin, gaze_z_origin - gaze_z_pt))

    # Setup output dataframe
    output_df = pd.DataFrame(output_df, index=tobii_df.index)

    # Save the gaze yaw features
    output_writer.save_tabular(output_df, "gaze_pitch_yaw")
...
def main():
    ...
    parser.add_argument(
        "--out_dir",
        type=str,
        default="dataset/Vehicle/No-Video/Resampled_previous_60_derived",
    ...
    config.features = {
        "experiment_tobii_frame": {
            ...
            "fps": 60,
            "columns": [
                "tobii_left_eye_gaze_origin_in_user_x",
                "tobii_left_eye_gaze_pt_in_user_x",
                "tobii_right_eye_gaze_origin_in_user_x",
                "tobii_right_eye_gaze_pt_in_user_x",
                "tobii_left_eye_gaze_origin_in_user_y",
                "tobii_left_eye_gaze_pt_in_user_y",
                "tobii_right_eye_gaze_origin_in_user_y",
                "tobii_right_eye_gaze_pt_in_user_y",
                "tobii_left_eye_gaze_origin_in_user_z",
                "tobii_left_eye_gaze_pt_in_user_z",
                "tobii_right_eye_gaze_origin_in_user_z",
                "tobii_right_eye_gaze_pt_in_user_z",
            ],
        },
    }
